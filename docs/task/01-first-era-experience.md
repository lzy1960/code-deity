...
- [x] **UI/UX 的全面实现与打磨 (用户决定跳过)**
    ...
    - **待办任务：**
        ...
        - [x] **统计页遥测补强：** 增加总游玩时长、累计 CP / RP、手动编写次数、奇点次数和挑战完成数等长期统计。
        - [x] **设置页面 (`pages/settings.vue`):** 对齐 `UI/settings_page/screen.png` 的设计。
        - [x] **增强史诗级叙事事件动画 (如奇点):** 实现了更具冲击力的入场和退场动画效果，并优化了白光效果使其更平滑。

- [x] **数值平衡性测试与调整 (初步验证完成)**
    - **现状：** 已完成首次重构流程的理论推演，并根据结果实装了新的“全局生产力加成”机制。随后通过实际游玩反馈，对“奇点筹备度”、编译发布反馈、奇点前节奏进行了体验验证，当前体感可接受。
    - **已完成:**
        - [x] 完成首次重构流程的理论推演与分析。
        - [x] 根据分析结果，废弃了旧的平衡方案，并实装了新的“全局生产力加成”机制。
        - [x] 完成实际游戏体验验证，确认筹备度增长、编译发布反馈、奇点前节奏没有明显阻塞。
    - **后续关注:**
        - [ ] 第二纪元深度游玩和第三纪元开发后，需要再次回归验证整体数值曲线。
