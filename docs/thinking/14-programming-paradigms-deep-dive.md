...
- **详细描述:** "你学会了手动管理内存，降低了资源开销。所有生成器的基础成本降低 20%。"
- **作用与影响:** 在 `game.ts` 的 `effectiveGeneratorConfig` 中，它会将所有生成器的 `baseCost` 乘以 `0.8`（降低20%）。这是一个直接提高购买效率的资源型技能，适合帮助玩家更快跨过高阶生成器的成本墙。
...
- **详细描述:** "你直接使用汇编语言编写核心代码，虽然过程极度痛苦，但回报是巨大的。所有生成器产出获得 x(重构次数 * 4) 加成，最高 x500。"
- **作用与影响:** 这是一个潜力巨大的成长性技能。在 `game.ts` 的 `generatorProduction` 中，它会将所有生成器的产出乘以 `min(500, refactorCount * 4)`。它奖励频繁重构，但通过上限避免后期完全失控。
...
- **详细描述:** "你掌握了可复用的软件设计解决方案，让你的代码结构更有弹性。所有生成器产出提升 20%。"
- **作用与影响:** 这是抽象学派后续两个强大技能的前置，同时提供一个稳定的 `x1.2` 全局产出加成，让学习该前置节点不会显得空转。

#### 2. 技能名称: `多态 (Polymorphism)`
- **详细描述:** "不同对象可以响应相同的消息，这让你的代码更加通用。二阶以上生成器会继承低一阶“购买奖励”增量的 5%，继承倍率最高 x3。"
- **作用与影响:** 这是一个精妙的级联机制。在 `game.ts` 的 `buy10Bonus` 计算中，当前生成器会从低一阶生成器的购买奖励中继承一小部分额外倍率，并设置上限避免爆炸。这鼓励玩家均衡发展，而不是只堆最高级生成器。
...
- **详细描述:** "你定义了清晰的应用程序接口，使得不同部分可以独立演进。架构过载惩罚效果降低 50%。"
- **作用与影响:** 这是一个缓解高阶 AI 核心堆叠惩罚的稳定性技能。在 `architecturalOverheadPenalty` 中，它会把惩罚因子减半，让后期继续扩张高阶生成器时更平滑。
...
- **详细描述:** "你优化了整个软件供应链，使得高阶概念的实现更为高效。'模块'（四阶生成器）产量的 5% 会直接转化为'类'（三阶生成器）。"
- **作用与影响:** 这是一个“跳线”加成。在 `game.ts` 的 `simulateProgress` 中，它让模块生产链额外回流到类，略微加速中前期结构补强。
...
#### 6. 技能名称: `代码生成 (Code Generation)`
- **详细描述:** "你开始让系统自动生成部分代码，手动超频基础收益从 5% CPS 提升至 8% CPS，且代码狂潮充能速度提升 50%。"
- **作用与影响:** 这是一个主动玩法强化技能。在 `manualClickPower` 中，它提高单次手动收益；在 `manualClick` 的代码狂潮充能逻辑中，它让主动点击更快进入爆发期。
