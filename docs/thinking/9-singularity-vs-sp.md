...
> 实现状态说明（2026-06-09）：
> 本文关于“奇点”和“SP”的概念区分仍然有效，但文中举例使用的奇点门槛 `1e308 CP` 已不是当前实现。
> 当前实际实现中，第一次技术奇点条件为 `CP >= 1e120` 且“有效奇点筹备度 >= 100”。
...
- **奇点 (Singularity)** 相当于“**毕业典礼**”。这是一个玩家必须通过努力（在当前实现中，例如满足 `1e120 CP` 与筹备度条件）才能触发的、标志着一个时代结束的**一次性事件**。
- **奇点算力 (SP)** 相当于“**学位证书**”。它是玩家完成“毕业”这个事件后，所获得的**有价值的、可用于开启新未来的资源**。毕业时的成绩越好（CP 超出当前门槛越多），“证书”的含金量就越高（获得的 SP 越多）。
...
| **如何获得** | 在当前实现中，当算力 (CP) 达到 `1e120` 且有效奇点筹备度达到 `100` 时，**触发**或**执行**这次重置事件。 | 在执行“奇点”重置的**那一刻**，根据当时的 CP 数量**计算并获得**。 |
...
玩家通过努力满足当前版本的奇点条件，然后**执行“奇点”**（事件），这个行为会重置游戏，并根据当时的 CP 总量，给予玩家一定数量的**“奇点算力 (SP)”**（资源）。随后，玩家便可以使用这些 SP 在全新的技能树中购买升级，正式开始他们作为“架构师”的新纪元。
