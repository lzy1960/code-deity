<template>
  <div class="space-y-3">
    ...
    <p class="text-gray-400">Automatically buy generators. Unlocks after your first Compile & Release.</p>
    <div v-for="generator in unlockedGenerators" :key="generator.id" class="flex items-center justify-between rounded-lg bg-[#191933] p-3">
      <span class="text-base font-medium">Auto-buy {{ generatorConfig(generator.id).name }}</span>
      <label class="relative inline-flex items-center cursor-pointer">
        <input type="checkbox" :checked="gameStore.automators[generator.id]" @change="toggleAutomator(generator.id)" class="sr-only peer">
