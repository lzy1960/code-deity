    ...
    <div v-if="modal.isRevealed.value && modal.gains.value" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" @click.self="modal.hide()">
      <div class="relative m-4 w-full max-w-md rounded-2xl bg-transparent p-0 shadow-2xl shadow-blue-500/20 overflow-hidden border border-blue-500/20">
        ...
        <div class="relative z-10 rounded-[14px] bg-gray-900 m-[2px] p-4">
          <h2 class="mb-3 text-center text-base font-bold text-blue-300">
          ...
          <div class="space-y-2 rounded-lg bg-black/30 p-3 text-center">
            <div>
              <p class="text-xs text-gray-400">{{ $t('common.gainFromRefactor') }}</p>
              <p class="text-lg font-bold text-green-400">+{{ formatNumber(modal.gains.value.potentialRpGain) }} RP</p>
            </div>
          ...
          <div class="mt-4 flex justify-between gap-3">
            ...
            <button
              class="flex-1 rounded-lg bg-blue-600 px-4 py-2 text-sm font-bold text-white shadow-lg shadow-blue-600/30 transition-colors hover:bg-blue-700"
              @click="modal.confirm()"
            >
              {{ $t('common.confirm') }}
            </button>
