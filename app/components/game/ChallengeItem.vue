<template>
  <div class="rounded-xl p-5 transition-all duration-300" :class="containerClass">
    ...
    <div class="mt-3 space-y-2 text-sm text-gray-300/80">
      <p><span class="font-semibold text-gray-200">{{ $t('common.rule') }}:</span> {{ rule }}</p>
      <p><span class="font-semibold text-gray-200">{{ $t('common.goal') }}:</span> {{ goal }}</p>
      <p class="font-bold text-cyan-400"><span class="font-semibold text-gray-200">{{ $t('common.reward') }}:</span> {{ reward }}</p>
    </div>
