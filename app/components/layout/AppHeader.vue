  ...
  <header class="sticky top-0 z-10 bg-[#101a23]/80 backdrop-blur-sm">
    <div class="flex items-center p-4 pb-2 justify-between">
      ...
      <div class="flex w-12 items-center justify-end gap-2">
        <NuxtLink v-if="authStore.isLoggedIn" to="/account" class="flex max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 w-10 bg-transparent text-white gap-2 text-base font-bold leading-normal tracking-[0.015em] min-w-0 p-0 hover:bg-white/10 transition-colors">
          <img :src="authStore.user?.user_metadata.avatar_url" alt="User Avatar" class="h-8 w-8 rounded-full object-cover">
        </NuxtLink>
        <NuxtLink v-else to="/account" class="flex max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 w-10 bg-transparent text-white gap-2 text-base font-bold leading-normal tracking-[0.015em] min-w-0 p-0 hover:bg-white/10 transition-colors">
          <Icon name="mdi:account-circle-outline" class="text-3xl" />
        </NuxtLink>
      </div>
    </div>
